...
func (s *Server) GetNeighbor(ctx context.Context, arg *api.Arguments) (*api.Peer, error) {
	...
	req := NewApiRequest(REQ_NEIGHBOR, arg.RouterId, rf, nil)
	s.bgpServerCh <- req
...
func (s *Server) GetNeighbors(_ *api.Arguments, stream api.Grpc_GetNeighborsServer) error {
	var rf bgp.RouteFamily
	req := NewApiRequest(REQ_NEIGHBORS, "", rf, nil)
	s.bgpServerCh <- req
...
func (s *Server) GetAdjRib(arg *api.Arguments, stream api.Grpc_GetAdjRibServer) error {
	...
	req := NewApiRequest(reqType, arg.RouterId, rf, nil)
	s.bgpServerCh <- req
...
func (s *Server) ModPath(stream api.Grpc_ModPathServer) error {
	for {
		...
		req := NewApiRequest(reqType, "", rf, arg.Path)
		s.bgpServerCh <- req
...
func NewApiRequest(reqType int, remoteAddr string, rf bgp.RouteFamily, d interface{}) *ApiRequest {
	r := &ApiRequest{
		RequestType: reqType,
		RouteFamily: rf,
...
func NewApiServer(port int, bgpServerCh chan *ApiRequest) *Server {
