	...
	if a.Actions != nil {
		...
		afs := []func() (table.Action, error){
			...
			func() (table.Action, error) {
				...
				return table.NewMedActionFromApiStruct(table.MedActionType(a.Actions.Med.Type), int(a.Actions.Med.Value)), nil
			},
			func() (table.Action, error) {
...
func defaultRouteType(d api.RouteAction) (table.RouteType, error) {
	switch d {
	case api.RouteAction_ACCEPT:
		return table.ROUTE_TYPE_ACCEPT, nil
	case api.RouteAction_REJECT:
		return table.ROUTE_TYPE_REJECT, nil
	default:
		return table.ROUTE_TYPE_NONE, fmt.Errorf("unknown route type %s", d.String())
	}
}
...
func (s *Server) AddPolicyAssignment(ctx context.Context, arg *api.AddPolicyAssignmentRequest) (*api.AddPolicyAssignmentResponse, error) {
	...
	def, err := defaultRouteType(arg.Assignment.Default)
	if err != nil {
		return nil, err
	}
	return &api.AddPolicyAssignmentResponse{}, s.bgpServer.AddPolicyAssignment(name, dir, toPolicyDefinition(arg.Assignment.Policies), def)
}
...
func (s *Server) ReplacePolicyAssignment(ctx context.Context, arg *api.ReplacePolicyAssignmentRequest) (*api.ReplacePolicyAssignmentResponse, error) {
	...
	return &api.ReplacePolicyAssignmentResponse{}, s.bgpServer.ReplacePolicyAssignment(name, dir, toPolicyDefinition(arg.Assignment.Policies), def)
}
