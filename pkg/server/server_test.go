...
func TestPeerRestartEORPerFamily(t *testing.T) {
	...
	runLocalRestartTestOp(t, s, func() {
		...
		case <-ended:
		default:
			t.Fatal("IPv4 LLGR timer not stopped")
		}
		select {
		case <-pending:
			t.Fatal("IPv6 LLGR timer stopped prematurely")
		default:
		}
	})
...
func TestPeerRestartLLGRExpiryAcrossReset(t *testing.T) {
	for _, synchronized := range []bool{false, true} {
		t.Run(map[bool]string{false: "before-eor", true: "after-eor"}[synchronized], func(t *testing.T) {
			...
			// Expiry of the ordinary restart period starts the LLGR timer.
			s.handleFSMMessage(p, &fsmMsg{
				MsgType: fsmMsgStateChange, MsgData: bgp.BGP_FSM_IDLE,
			...
			s.handleFSMMessage(p, &fsmMsg{
				MsgType: fsmMsgStateChange, MsgData: bgp.BGP_FSM_ACTIVE,
			...
			if synchronized {
				...
				require.Zero(t, retained, "reset after LLGR timer expiry but before EOR must remove refreshed routes immediately")
			}
			s.handleFSMMessage(p, &fsmMsg{
				MsgType: fsmMsgStateChange, MsgData: bgp.BGP_FSM_IDLE,
				StateReason: &fsmStateReason{Type: fsmRestartTimerExpired},
			...
			require.Equal(t, synchronized, restarted, "only EOR permits another LLGR timer")
		})
	}
}
...
func TestLocalRestartSecondaryReconnectDump(t *testing.T) {
	...
	s.handleFSMMessage(p, &fsmMsg{
		MsgType: fsmMsgStateChange, MsgData: bgp.BGP_FSM_ESTABLISHED,
...
func TestPeerRestartLLGRNewTimerAfterOneAFEOR(t *testing.T) {
	...
	require.NotNil(t, newV4, "synchronized AF needs its own new LLGR timer")
	require.NotEqual(t, oldV4, newV4)
	require.Equal(t, oldV6, newV6, "unsynchronized AF must retain its original LLGR timer")
	require.Eventually(t, func() bool {
