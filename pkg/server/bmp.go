...
func (b *bmpClient) loop() {
	for {
		...
		if func() bool {
			...
			if b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_BOTH {
				b.s.logger.Warn("both option for route-monitoring-policy is obsoleted", log.Fields{"Topic": "bmp"})
			}
			if b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_PRE_POLICY || b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_ALL {
				ops = append(ops, watchUpdate(true, ""))
			}
			if b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_POST_POLICY || b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_ALL {
				ops = append(ops, watchPostUpdate(true, ""))
			}
			if b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_LOCAL_RIB || b.c.RouteMonitoringPolicy == bgpconfig.BMP_ROUTE_MONITORING_POLICY_TYPE_ALL {
...
func (b *bmpClientManager) addServer(c *bgpconfig.BmpServerConfig) error {
...
func (b *bmpClientManager) deleteServer(c *bgpconfig.BmpServerConfig) error {
	host := net.JoinHostPort(c.Address, strconv.Itoa(int(c.Port)))
