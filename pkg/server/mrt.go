...
func (m *mrtWriter) loop() error {
	ops := []watchOption{}
	switch m.c.DumpType {
	case gobgp.MRT_TYPE_UPDATES:
		ops = append(ops, watchUpdate(false, "", ""))
	case gobgp.MRT_TYPE_TABLE:
	...
	for {
		serialize := func(ev watchEvent) []*mrt.MRTMessage {
			...
			case *watchEventTable:
				...
				neighborMap := make(map[string]*gobgp.Neighbor)
		...
		case e := <-w.Event():
			drain(e)
			if m.c.DumpType == gobgp.MRT_TYPE_TABLE && m.rotationInterval != 0 {
				rotate()
			}
		case <-rotator.C:
			if m.c.DumpType == gobgp.MRT_TYPE_UPDATES {
				rotate()
			} else {
...
func newMrtWriter(s *BgpServer, c *gobgp.MrtConfig, rInterval, dInterval uint64) (*mrtWriter, error) {
...
func (m *mrtManager) enable(c *gobgp.MrtConfig) error {
	...
	if c.DumpType == gobgp.MRT_TYPE_TABLE {
		if rInterval == 0 {
	...
	} else if c.DumpType == gobgp.MRT_TYPE_UPDATES {
...
func (m *mrtManager) disable(c *gobgp.MrtConfig) error {
