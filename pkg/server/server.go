...
func filterpath(peer *peer, path, old *table.Path) *table.Path {
	...
	if !peer.isRouteServerClient() && isASLoop(peer, path) {
		...
		if path.IsLocal() && !peer.allowAsPathLoopLocal() || !path.IsLocal() {
			if !path.IsWithdraw && old != nil {
				...
				// A2 and path stuck. So in this case B2 should send explicit withdraw for old path.
				return old.Clone(true)
			}
			return nil
		}
...
func (s *BgpServer) AddVrf(ctx context.Context, r *api.AddVrfRequest) error {
	...
	return s.mgmtOperation(func() error {
		...
		pi := &table.PeerInfo{
			...
			LocalID: netip.MustParseAddr(s.bgpConfig.Global.Config.RouterId),
		}
