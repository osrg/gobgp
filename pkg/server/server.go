...
func filterpath(peer *peer, path, old *table.Path) *table.Path {
	...
	peer.fsm.lock.RUnlock()
	if y && path.GetRouteFamily() != bgp.RF_RTC_UC && !peer.adjRibIn.HasRTinRtcTable(bgp.DefaultRT) {
		ignore := true
		for _, ext := range path.GetExtCommunities() {
			key, err := bgp.ExtCommRouteTargetKey(ext)
			if err != nil {
				continue
			}
...
func (s *BgpServer) filterPathListForWithdrawnRT(rt uint64, peer *peer, rfList []bgp.RouteFamily) []*table.Path {
