...
func NewBfdPeer(ps peerState, logger *slog.Logger, peerAddress string, config oc.BfdConfig) *bfdPeer {
	...
	if config.RxInterval > 0 {
		p.rxInterval = time.Duration(config.RxInterval) * time.Millisecond
	}

	if config.TxInterval > 0 {
		p.txInterval = time.Duration(config.TxInterval) * time.Millisecond
	}
