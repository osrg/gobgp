...
func (s *bfdServer) GetServerStats() *api.BfdServerState {
	return &api.BfdServerState{
...
func (s *bfdServer) getPeerState(address netip.Addr) *bfdPeerState {
	...
	return &bfdPeerState{
		...
		state: api.BfdPeerState{
			State:                peer.state.Load(),
			ReceivedPacket:       peer.stats.rxPacket.Load(),
			SentPacket:           peer.stats.txPacket.Load(),
			SentDrop:             peer.stats.txDrop.Load(),
			SentError:            peer.stats.txError.Load(),
			InvalidDiscriminator: peer.stats.invalidDiscriminator.Load(),
			Expired:              peer.stats.expired.Load(),
			BadInitPacket:        peer.stats.badInitPacket.Load(),
		},
	}
}
