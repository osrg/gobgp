		...
		t.Run(prefix, func(t *testing.T) {
			...
			require.NoError(t, s.mgmtOperation(func() error {
				for _, path := range []*table.Path{loop, valid} {
					accepted, withdrawals, _, limit := p.handleUpdate(&fsmMsg{MsgData: asPathTestUpdate(t, path), timestamp: time.Now()}, nil)
					assert.False(t, limit)
					s.propagateUpdate(p, append(accepted, withdrawals...))
				}
				return nil
			}, true))
			check := func(want int) {
			...
			require.NoError(t, s.mgmtOperation(func() error {
				paths, _, _, _ := p.handleUpdate(&fsmMsg{MsgData: asPathTestUpdate(t, valid.Clone(true)), timestamp: time.Now()}, nil)
...
func TestSoftResetInPreservesLoopChecksAndImportPolicy(t *testing.T) {
	...
	require.NoError(t, s.mgmtOperation(func() error {
		for _, path := range paths {
			accepted, _, _, _ := p.handleUpdate(&fsmMsg{MsgData: asPathTestUpdate(t, path), timestamp: time.Now()}, nil)
	...
	require.NoError(t, s.mgmtOperation(func() error {
		...
		withdrawals, _, _, _ := p.handleUpdate(&fsmMsg{MsgData: asPathTestUpdate(t, paths[2]), timestamp: time.Now(), handling: bgp.ERROR_HANDLING_TREAT_AS_WITHDRAW}, nil)
