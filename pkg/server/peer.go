...
type peerGroup struct {
	Conf             *gobgp.PeerGroup
	members          map[string]gobgp.Neighbor
	dynamicNeighbors map[string]*gobgp.DynamicNeighbor
}

func newPeerGroup(c *gobgp.PeerGroup) *peerGroup {
	return &peerGroup{
		Conf:             c,
		members:          make(map[string]gobgp.Neighbor),
		dynamicNeighbors: make(map[string]*gobgp.DynamicNeighbor),
	}
}

func (pg *peerGroup) AddMember(c gobgp.Neighbor) {
	pg.members[c.State.NeighborAddress] = c
}

func (pg *peerGroup) DeleteMember(c gobgp.Neighbor) {
	delete(pg.members, c.State.NeighborAddress)
}

func (pg *peerGroup) AddDynamicNeighbor(c *gobgp.DynamicNeighbor) {
...
func newDynamicPeer(g *gobgp.Global, neighborAddress string, pg *gobgp.PeerGroup, loc *table.TableManager, policy *table.RoutingPolicy, logger log.Logger) *peer {
	conf := gobgp.Neighbor{
	...
	if err := gobgp.OverwriteNeighborConfigWithPeerGroup(&conf, pg); err != nil {
	...
	if err := gobgp.SetDefaultNeighborConfigValues(&conf, pg, g); err != nil {
...
func newPeer(g *gobgp.Global, conf *gobgp.Neighbor, loc *table.TableManager, policy *table.RoutingPolicy, logger log.Logger) *peer {
	peer := &peer{
		localRib:          loc,
		policy:            policy,
	...
	rfs, _ := gobgp.AfiSafis(conf.AfiSafis).ToRfList()
...
func (peer *peer) isIBGPPeer() bool {
	peer.fsm.lock.RLock()
	defer peer.fsm.lock.RUnlock()
	return peer.fsm.pConf.State.PeerType == gobgp.PEER_TYPE_INTERNAL
}
...
func (peer *peer) configuredRFlist() []bgp.RouteFamily {
	peer.fsm.lock.RLock()
	defer peer.fsm.lock.RUnlock()
	rfs, _ := gobgp.AfiSafis(peer.fsm.pConf.AfiSafis).ToRfList()
	return rfs
}
...
func (peer *peer) doPrefixLimit(k bgp.RouteFamily, c *gobgp.PrefixLimitConfig) *bgp.BGPMessage {
...
func (peer *peer) updatePrefixLimitConfig(c []gobgp.AfiSafi) error {
	peer.fsm.lock.RLock()
	...
	m := make(map[bgp.RouteFamily]gobgp.PrefixLimitConfig)
	for _, e := range x {
