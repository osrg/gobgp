...
[[keychains]]
  ...
  [[keychains.keys]]
    [keychains.keys.config]
      key-id = 1
      receive-id = 11
      crypto-algorithm = "hmac-sha-1-96"
...
`)
	...
	replacement.Keychains[0].Keys[0].Config.CryptoAlgorithm = oc.CRYPTO_TYPE_AES_128_CMAC_96
	replacement.Keychains[0].Keys[0].Config.ExcludeTcpOptions = true
	current, err = UpdateConfig(context.Background(), bgpServer, current, &replacement)
	require.NoError(t, err)

	listed = listTcpAoKeychains(t, bgpServer)
	require.Len(t, listed["fabric"].Keys, 1)
	assert.Equal(t, api.TcpAoAlgorithm_TCP_AO_ALGORITHM_AES_128_CMAC_96, listed["fabric"].Keys[0].Algorithm)
	assert.True(t, listed["fabric"].Keys[0].ExcludeTcpOptions)
...
[[keychains]]
  ...
  [[keychains.keys]]
    [keychains.keys.config]
      key-id = 2
      receive-id = 12
      crypto-algorithm = "aes-128-cmac-96"
...
`)
	...
	listed = listTcpAoKeychains(t, bgpServer)
	assert.NotContains(t, listed, "edge")
}
