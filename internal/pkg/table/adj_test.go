	...
	for _, modified := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			for range 8 {
				...
				path, withdrawals = adj.SetRejected(previous, rejected)
				assert.NotSame(t, previous, path)
				if rejected {
					require.Len(t, withdrawals, 1)
					assert.Equal(t, localID, withdrawals[0].LocalID())
				} else {
				...
				if rejected {
					assert.Zero(t, path.LocalID())
				} else {
					assert.Equal(t, localID, path.LocalID())
				}
				...
				same, withdrawals := adj.SetRejected(path, rejected)
				assert.Same(t, path, same)
