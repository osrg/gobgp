...
func (o MatchOption) ConvertToMatchSetOptionsRestrictedType() gobgp.MatchSetOptionsRestrictedType {
	switch o {
	case MATCH_OPTION_ANY:
...
var CommunityOptionNameMap = map[gobgp.BgpSetCommunityOptionType]string{
	gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_ADD:     "add",
	gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE:  "remove",
	gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REPLACE: "replace",
}

var CommunityOptionValueMap = map[string]gobgp.BgpSetCommunityOptionType{
	CommunityOptionNameMap[gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_ADD]:     gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_ADD,
	CommunityOptionNameMap[gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE]:  gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE,
	CommunityOptionNameMap[gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REPLACE]: gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REPLACE,
}
...
func NewMatchOption(c interface{}) (MatchOption, error) {
	switch t := c.(type) {
	case gobgp.MatchSetOptionsType:
		t = t.DefaultAsNeeded()
		switch t {
		case gobgp.MATCH_SET_OPTIONS_TYPE_ANY:
			return MATCH_OPTION_ANY, nil
		case gobgp.MATCH_SET_OPTIONS_TYPE_ALL:
			return MATCH_OPTION_ALL, nil
		case gobgp.MATCH_SET_OPTIONS_TYPE_INVERT:
			return MATCH_OPTION_INVERT, nil
		}
	case gobgp.MatchSetOptionsRestrictedType:
		t = t.DefaultAsNeeded()
		switch t {
		case gobgp.MATCH_SET_OPTIONS_RESTRICTED_TYPE_ANY:
			return MATCH_OPTION_ANY, nil
		case gobgp.MATCH_SET_OPTIONS_RESTRICTED_TYPE_INVERT:
			return MATCH_OPTION_INVERT, nil
		}
	}
...
func NewPrefix(c gobgp.Prefix) (*Prefix, error) {
...
func (s *PrefixSet) ToConfig() *gobgp.PrefixSet {
	list := make([]gobgp.Prefix, 0, s.tree.Size())
	s.tree.Walk(nil, func(_ *net.IPNet, v interface{}) bool {
		ps := v.([]*Prefix)
		for _, p := range ps {
			list = append(list, gobgp.Prefix{IpPrefix: p.PrefixString(), MasklengthRange: fmt.Sprintf("%d..%d", p.MasklengthRangeMin, p.MasklengthRangeMax)})
		}
		return true
	})
	return &gobgp.PrefixSet{
...
func NewPrefixSet(c gobgp.PrefixSet) (*PrefixSet, error) {
...
func (s *NeighborSet) ToConfig() *gobgp.NeighborSet {
	return &gobgp.NeighborSet{
...
func NewNeighborSet(c gobgp.NeighborSet) (*NeighborSet, error) {
...
func (s *AsPathSet) ToConfig() *gobgp.AsPathSet {
	return &gobgp.AsPathSet{
...
func NewAsPathSet(c gobgp.AsPathSet) (*AsPathSet, error) {
...
func (s *CommunitySet) ToConfig() *gobgp.CommunitySet {
	return &gobgp.CommunitySet{
...
func NewCommunitySet(c gobgp.CommunitySet) (*CommunitySet, error) {
...
func (s *ExtCommunitySet) ToConfig() *gobgp.ExtCommunitySet {
	return &gobgp.ExtCommunitySet{
...
func NewExtCommunitySet(c gobgp.ExtCommunitySet) (*ExtCommunitySet, error) {
...
func (s *LargeCommunitySet) ToConfig() *gobgp.LargeCommunitySet {
	return &gobgp.LargeCommunitySet{
...
func NewLargeCommunitySet(c gobgp.LargeCommunitySet) (*LargeCommunitySet, error) {
...
func NewPrefixCondition(c gobgp.MatchPrefixSet) (*PrefixCondition, error) {
	if c.PrefixSet == "" {
		return nil, nil
	}
...
func NewNeighborCondition(c gobgp.MatchNeighborSet) (*NeighborCondition, error) {
	if c.NeighborSet == "" {
		return nil, nil
	}
	o, err := NewMatchOption(c.MatchSetOptions)
...
func NewAsPathCondition(c gobgp.MatchAsPathSet) (*AsPathCondition, error) {
	if c.AsPathSet == "" {
		return nil, nil
	}
	o, err := NewMatchOption(c.MatchSetOptions)
...
func NewCommunityCondition(c gobgp.MatchCommunitySet) (*CommunityCondition, error) {
	if c.CommunitySet == "" {
		return nil, nil
	}
	o, err := NewMatchOption(c.MatchSetOptions)
...
func NewExtCommunityCondition(c gobgp.MatchExtCommunitySet) (*ExtCommunityCondition, error) {
...
func NewLargeCommunityCondition(c gobgp.MatchLargeCommunitySet) (*LargeCommunityCondition, error) {
...
func NewAsPathLengthCondition(c gobgp.AsPathLength) (*AsPathLengthCondition, error) {
	...
	if i := c.Operator.ToInt(); i < 0 {
		...
		// for simple configuration. see gobgp.AttributeComparison definition
...
type RpkiValidationCondition struct {
	result gobgp.RpkiValidationResultType
}
...
func NewRpkiValidationCondition(c gobgp.RpkiValidationResultType) (*RpkiValidationCondition, error) {
	if c == gobgp.RpkiValidationResultType("") || c == gobgp.RPKI_VALIDATION_RESULT_TYPE_NONE {
		return nil, nil
	}
...
func (c *RouteTypeCondition) Evaluate(path *Path, _ *PolicyOptions) bool {
	switch c.typ {
	case gobgp.ROUTE_TYPE_LOCAL:
		return path.IsLocal()
	case gobgp.ROUTE_TYPE_INTERNAL:
		return !path.IsLocal() && path.IsIBGP()
	case gobgp.ROUTE_TYPE_EXTERNAL:
		return !path.IsLocal() && !path.IsIBGP()
	}
	return false
}
...
func NewRouteTypeCondition(c gobgp.RouteType) (*RouteTypeCondition, error) {
	if string(c) == "" || c == gobgp.ROUTE_TYPE_NONE {
		return nil, nil
	}
...
func NewAfiSafiInCondition(afiSafInConfig []gobgp.AfiSafiType) (*AfiSafiInCondition, error) {
...
func NewRoutingAction(c gobgp.RouteDisposition) (*RoutingAction, error) {
	var accept bool
	switch c {
	case gobgp.RouteDisposition(""), gobgp.ROUTE_DISPOSITION_NONE:
		return nil, nil
	case gobgp.ROUTE_DISPOSITION_ACCEPT_ROUTE:
		accept = true
	case gobgp.ROUTE_DISPOSITION_REJECT_ROUTE:
		accept = false
	default:
...
type CommunityAction struct {
	action     gobgp.BgpSetCommunityOptionType
	list       []uint32
...
func (a *CommunityAction) Apply(path *Path, _ *PolicyOptions) (*Path, error) {
	switch a.action {
	case gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_ADD:
		path.SetCommunities(a.list, false)
	case gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE:
		RegexpRemoveCommunities(path, a.removeList)
	case gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REPLACE:
		path.SetCommunities(a.list, true)
	}
	return path, nil
}

func (a *CommunityAction) ToConfig() *gobgp.SetCommunity {
	...
	return &gobgp.SetCommunity{
		Options:            string(a.action),
		SetCommunityMethod: gobgp.SetCommunityMethod{CommunitiesList: cs},
	}
}
...
func NewCommunityAction(c gobgp.SetCommunity) (*CommunityAction, error) {
	...
	if a == gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE {
...
func (a *ExtCommunityAction) ToConfig() *gobgp.SetExtCommunity {
	cs := make([]string, 0, len(a.list)+len(a.removeList))
	f := func(idx int, arg string) string {
	...
	return &gobgp.SetExtCommunity{
		Options: string(a.action),
		SetExtCommunityMethod: gobgp.SetExtCommunityMethod{
			CommunitiesList: cs,
		},
	}
}
...
func NewExtCommunityAction(c gobgp.SetExtCommunity) (*ExtCommunityAction, error) {
	a, ok := CommunityOptionValueMap[strings.ToLower(c.Options)]
	if !ok {
...
func (a *LargeCommunityAction) ToConfig() *gobgp.SetLargeCommunity {
	cs := make([]string, 0, len(a.list)+len(a.removeList))
	for _, comm := range a.list {
	...
	return &gobgp.SetLargeCommunity{
		SetLargeCommunityMethod: gobgp.SetLargeCommunityMethod{CommunitiesList: cs},
		Options:                 gobgp.BgpSetCommunityOptionType(a.action),
	}
}
...
func NewLargeCommunityAction(c gobgp.SetLargeCommunity) (*LargeCommunityAction, error) {
...
func (a *MedAction) ToConfig() gobgp.BgpSetMedType {
	if a.action == MED_ACTION_MOD && a.value > 0 {
		return gobgp.BgpSetMedType(fmt.Sprintf("+%d", a.value))
	}
	return gobgp.BgpSetMedType(fmt.Sprintf("%d", a.value))
}
...
func NewMedAction(c gobgp.BgpSetMedType) (*MedAction, error) {
	if string(c) == "" {
		return nil, nil
	}
...
func (a *AsPathPrependAction) ToConfig() *gobgp.SetAsPathPrepend {
	return &gobgp.SetAsPathPrepend{
...
func NewAsPathPrependAction(action gobgp.SetAsPathPrepend) (*AsPathPrependAction, error) {
...
func (a *NexthopAction) ToConfig() gobgp.BgpNextHopType {
	if a.self {
		return gobgp.BgpNextHopType("self")
	}
	if a.unchanged {
		return gobgp.BgpNextHopType("unchanged")
	}
	return gobgp.BgpNextHopType(a.value.String())
}
...
func NewNexthopAction(c gobgp.BgpNextHopType) (*NexthopAction, error) {
...
func (s *Statement) ToConfig() *gobgp.Statement {
	return &gobgp.Statement{
		Name: s.Name,
		Conditions: func() gobgp.Conditions {
			cond := gobgp.Conditions{}
			for _, c := range s.Conditions {
				switch v := c.(type) {
				case *PrefixCondition:
					cond.MatchPrefixSet = gobgp.MatchPrefixSet{PrefixSet: v.set.Name(), MatchSetOptions: v.option.ConvertToMatchSetOptionsRestrictedType()}
				case *NeighborCondition:
					cond.MatchNeighborSet = gobgp.MatchNeighborSet{NeighborSet: v.set.Name(), MatchSetOptions: v.option.ConvertToMatchSetOptionsRestrictedType()}
				case *AsPathLengthCondition:
					cond.BgpConditions.AsPathLength = gobgp.AsPathLength{Operator: gobgp.IntToAttributeComparisonMap[int(v.operator)], Value: v.length}
				case *AsPathCondition:
					cond.BgpConditions.MatchAsPathSet = gobgp.MatchAsPathSet{AsPathSet: v.set.Name(), MatchSetOptions: gobgp.IntToMatchSetOptionsTypeMap[int(v.option)]}
				case *CommunityCondition:
					cond.BgpConditions.MatchCommunitySet = gobgp.MatchCommunitySet{CommunitySet: v.set.Name(), MatchSetOptions: gobgp.IntToMatchSetOptionsTypeMap[int(v.option)]}
				case *ExtCommunityCondition:
					cond.BgpConditions.MatchExtCommunitySet = gobgp.MatchExtCommunitySet{ExtCommunitySet: v.set.Name(), MatchSetOptions: gobgp.IntToMatchSetOptionsTypeMap[int(v.option)]}
				case *LargeCommunityCondition:
					cond.BgpConditions.MatchLargeCommunitySet = gobgp.MatchLargeCommunitySet{LargeCommunitySet: v.set.Name(), MatchSetOptions: gobgp.IntToMatchSetOptionsTypeMap[int(v.option)]}
				...
				case *AfiSafiInCondition:
					res := make([]gobgp.AfiSafiType, 0, len(v.routeFamilies))
					for _, rf := range v.routeFamilies {
						res = append(res, gobgp.AfiSafiType(rf.String()))
					}
		...
		Actions: func() gobgp.Actions {
			act := gobgp.Actions{}
			if s.RouteAction != nil && !reflect.ValueOf(s.RouteAction).IsNil() {
				a := s.RouteAction.(*RoutingAction)
				if a.AcceptRoute {
					act.RouteDisposition = gobgp.ROUTE_DISPOSITION_ACCEPT_ROUTE
				} else {
					act.RouteDisposition = gobgp.ROUTE_DISPOSITION_REJECT_ROUTE
				}
			} else {
				act.RouteDisposition = gobgp.ROUTE_DISPOSITION_NONE
			}
...
func NewStatement(c gobgp.Statement) (*Statement, error) {
	if c.Name == "" {
...
func (p *Policy) ToConfig() *gobgp.PolicyDefinition {
	ss := make([]gobgp.Statement, 0, len(p.Statements))
...
func NewPolicy(c gobgp.PolicyDefinition) (*Policy, error) {
	if c.Name == "" {
...
func (r *RoutingPolicy) getAssignmentFromConfig(dir PolicyDirection, a gobgp.ApplyPolicy) ([]*Policy, RouteType, error) {
	var names []string
	var cdef gobgp.DefaultPolicyType
	...
	if cdef == gobgp.DEFAULT_POLICY_TYPE_REJECT_ROUTE {
...
func (r *RoutingPolicy) reload(c gobgp.RoutingPolicy) error {
...
func (r *RoutingPolicy) GetDefinedSet(typ DefinedType, name string) (*gobgp.DefinedSets, error) {
	...
	sets := &gobgp.DefinedSets{
		PrefixSets:   make([]gobgp.PrefixSet, 0),
		NeighborSets: make([]gobgp.NeighborSet, 0),
		BgpDefinedSets: gobgp.BgpDefinedSets{
			CommunitySets:      make([]gobgp.CommunitySet, 0),
			ExtCommunitySets:   make([]gobgp.ExtCommunitySet, 0),
			LargeCommunitySets: make([]gobgp.LargeCommunitySet, 0),
			AsPathSets:         make([]gobgp.AsPathSet, 0),
		},
	}
	for _, s := range dl {
		if name != "" && s.Name() != name {
...
func (r *RoutingPolicy) GetStatement(name string) []*gobgp.Statement {
	r.mu.RLock()
	defer r.mu.RUnlock()

	l := make([]*gobgp.Statement, 0, len(r.statementMap))
...
func (r *RoutingPolicy) GetPolicy(name string) []*gobgp.PolicyDefinition {
	ps := func() Policies {
		r.mu.RLock()
		defer r.mu.RUnlock()
	...
	l := make([]*gobgp.PolicyDefinition, 0, len(ps))
	for _, p := range ps {
...
func (r *RoutingPolicy) AddPolicyAssignment(id string, dir PolicyDirection, policies []*gobgp.PolicyDefinition, def RouteType) (err error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RoutingPolicy) DeletePolicyAssignment(id string, dir PolicyDirection, policies []*gobgp.PolicyDefinition, all bool) (err error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RoutingPolicy) SetPolicyAssignment(id string, dir PolicyDirection, policies []*gobgp.PolicyDefinition, def RouteType) (err error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RoutingPolicy) Initialize() error {
	...
	if err := r.reload(gobgp.RoutingPolicy{}); err != nil {
...
func (r *RoutingPolicy) setPeerPolicy(id string, c gobgp.ApplyPolicy) {
...
func (r *RoutingPolicy) SetPeerPolicy(peerId string, c gobgp.ApplyPolicy) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *RoutingPolicy) Reset(rp *gobgp.RoutingPolicy, ap map[string]gobgp.ApplyPolicy) error {
	if rp == nil {
...
func CanImportToVrf(v *Vrf, path *Path) bool {
	...
	set, _ := NewExtCommunitySet(gobgp.ExtCommunitySet{
		ExtCommunitySetName: v.Name,
		ExtCommunityList:    f(v.ImportRt),
	})
	matchSet := gobgp.MatchExtCommunitySet{
...
func toStatementApi(s *gobgp.Statement) *api.Statement {
	cs := &api.Conditions{}
	...
	as := &api.Actions{
		...
		Community: func() *api.CommunityAction {
			...
			return &api.CommunityAction{
				Type:        api.CommunityAction_Type(gobgp.BgpSetCommunityOptionTypeToIntMap[gobgp.BgpSetCommunityOptionType(s.Actions.BgpActions.SetCommunity.Options)]),
		...
		ExtCommunity: func() *api.CommunityAction {
			...
			return &api.CommunityAction{
				Type:        api.CommunityAction_Type(gobgp.BgpSetCommunityOptionTypeToIntMap[gobgp.BgpSetCommunityOptionType(s.Actions.BgpActions.SetExtCommunity.Options)]),
		...
		LargeCommunity: func() *api.CommunityAction {
			...
			return &api.CommunityAction{
				Type:        api.CommunityAction_Type(gobgp.BgpSetCommunityOptionTypeToIntMap[gobgp.BgpSetCommunityOptionType(s.Actions.BgpActions.SetLargeCommunity.Options)]),
...
func ToPolicyApi(p *gobgp.PolicyDefinition) *api.Policy {
	return &api.Policy{
		Name: p.Name,
...
func NewAPIRoutingPolicyFromConfigStruct(c *gobgp.RoutingPolicy) (*api.RoutingPolicy, error) {
	definedSets, err := gobgp.NewAPIDefinedSetsFromConfigStruct(&c.DefinedSets)
	if err != nil {
		return nil, err
	}
