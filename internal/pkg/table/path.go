...
type Validation struct {
	Status          gobgp.RpkiValidationResultType
...
func UpdatePathAttrs(logger log.Logger, global *gobgp.Global, peer *gobgp.Neighbor, info *PeerInfo, original *Path) *Path {
	...
	for _, a := range path.GetPathAttrs() {
		if _, y := bgp.PathAttrFlags[a.GetType()]; !y {
			...
			case bgp.BGP_ATTR_TYPE_CLUSTER_LIST, bgp.BGP_ATTR_TYPE_ORIGINATOR_ID:
				if !(peer.State.PeerType == gobgp.PEER_TYPE_INTERNAL && peer.RouteReflector.Config.RouteReflectorClient) {
	...
	if peer.State.PeerType == gobgp.PEER_TYPE_EXTERNAL {
		// NEXTHOP handling
	...
	} else if peer.State.PeerType == gobgp.PEER_TYPE_INTERNAL {
...
func (path *Path) RemovePrivateAS(localAS uint32, option gobgp.RemovePrivateAsOption) {
	original := path.GetAsPath()
	if original == nil {
		return
	}
	switch option {
	case gobgp.REMOVE_PRIVATE_AS_OPTION_ALL, gobgp.REMOVE_PRIVATE_AS_OPTION_REPLACE:
		...
		for _, param := range original.Value {
			...
			for _, as := range asList {
				if isPrivateAS(as) {
					if option == gobgp.REMOVE_PRIVATE_AS_OPTION_REPLACE {
