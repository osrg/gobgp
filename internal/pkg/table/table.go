...
func NlriRouteTargetKey(rtNlri *bgp.RouteTargetMembershipNLRI) (uint64, error) {
	if rtNlri.RouteTarget == nil {
		return DefaultRT, nil
	}
	return ExtCommRouteTargetKey(rtNlri.RouteTarget)
}

func ExtCommRouteTargetKey(rtExtComm bgp.ExtendedCommunityInterface) (uint64, error) {
	if rtExtComm == nil {
		return 0, ErrNilCommunity
	}
	switch rt := rtExtComm.(type) {
