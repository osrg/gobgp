...
func TestPrefixCalcurateNoRange(t *testing.T) {
	...
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/24", MasklengthRange: ""})
	match1 := pl1.Match(path)
	assert.Equal(t, true, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/23", MasklengthRange: ""})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
...
func TestPrefixCalcurateAddress(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.11.0.0/16", MasklengthRange: "21..24"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
	match2 := pl2.Match(path)
...
func TestPrefixCalcurateLength(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.64.0/24", MasklengthRange: "21..24"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.64.0/16", MasklengthRange: "21..24"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthRange(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..23"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "25..26"})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
	match3 := pl3.Match(path)
...
func TestPrefixCalcurateNoRangeIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: ""})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123:1::/64", MasklengthRange: ""})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
	pl3, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "64..80"})
	match3 := pl3.Match(path)
	assert.Equal(t, true, match3)
}
...
func TestPrefixCalcurateAddressIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:128::/48", MasklengthRange: "64..80"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "64..80"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123:64::/64", MasklengthRange: "64..80"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123:64::/48", MasklengthRange: "64..80"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthRangeIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "62..63"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "65..66"})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(gobgp.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "63..65"})
	match3 := pl3.Match(path)
	assert.Equal(t, true, match3)
}
...
func TestPolicyNotMatch(t *testing.T) {
	...
	ds.PrefixSets = []gobgp.PrefixSet{ps}
	ds.NeighborSets = []gobgp.NeighborSet{ns}
...
func TestPolicyDifferentRoutefamilyOfPathAndPolicy(t *testing.T) {
	...
	ds := gobgp.DefinedSets{}
	ds.PrefixSets = []gobgp.PrefixSet{psIPv4, psIPv6}
	ds.NeighborSets = []gobgp.NeighborSet{nsIPv4, nsIPv6}
...
func TestAsPathLengthConditionEvaluate(t *testing.T) {
	...
	asPathLength := gobgp.AsPathLength{
		Operator: "eq",
		Value:    5,
	}
...
func TestAsPathConditionEvaluate(t *testing.T) {
	...
	// create match condition
	asPathSet1 := gobgp.AsPathSet{
		AsPathSetName: "asset1",
		AsPathList:    []string{"^65001"},
	}

	asPathSet2 := gobgp.AsPathSet{
		AsPathSetName: "asset2",
		AsPathList:    []string{"65005$"},
	}

	asPathSet3 := gobgp.AsPathSet{
		AsPathSetName: "asset3",
		AsPathList:    []string{"65004", "65005$"},
	}

	asPathSet4 := gobgp.AsPathSet{
		AsPathSetName: "asset4",
		AsPathList:    []string{"65000$"},
	}

	asPathSet5 := gobgp.AsPathSet{
		AsPathSetName: "asset5",
		AsPathList:    []string{"65010"},
	}

	asPathSet6 := gobgp.AsPathSet{
		AsPathSetName: "asset6",
		AsPathList:    []string{"^65010$"},
	}

	m := make(map[string]DefinedSet)
	for _, s := range []gobgp.AsPathSet{asPathSet1, asPathSet2, asPathSet3,
		asPathSet4, asPathSet5, asPathSet6} {
		a, _ := NewAsPathSet(s)
		m[s.AsPathSetName] = a
	}

	createAspathC := func(name string, option gobgp.MatchSetOptionsType) *AsPathCondition {
		matchSet := gobgp.MatchAsPathSet{}
	...
	p1 := createAspathC("asset1", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createAspathC("asset2", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createAspathC("asset3", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createAspathC("asset4", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createAspathC("asset5", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset3", gobgp.MATCH_SET_OPTIONS_TYPE_ALL)
	p8 := createAspathC("asset3", gobgp.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestMultipleAsPathConditionEvaluate(t *testing.T) {
	...
	p5 := createAspathC("asset5", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset7", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createAspathC("asset8", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p9 := createAspathC("asset9", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
...
func TestAsPathCondition(t *testing.T) {
	...
	for k, v := range tests {
		s, _ := NewAsPathSet(gobgp.AsPathSet{
			AsPathSetName: k,
			AsPathList:    []string{k},
		})
		c, _ := NewAsPathCondition(gobgp.MatchAsPathSet{
			AsPathSet:       k,
...
func TestAsPathConditionWithOtherCondition(t *testing.T) {
	...
	ds.PrefixSets = []gobgp.PrefixSet{ps}
	ds.NeighborSets = []gobgp.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []gobgp.AsPathSet{asPathSet}
...
func TestAs4PathConditionEvaluateMixedWith2byteAS(t *testing.T) {
	...
	p1 := createAspathC("asset1", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createAspathC("asset2", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createAspathC("asset3", gobgp.MATCH_SET_OPTIONS_TYPE_ALL)
	p4 := createAspathC("asset4", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createAspathC("asset5", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset7", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
...
func TestCommunityConditionEvaluate(t *testing.T) {
	...
	for _, c := range []gobgp.CommunitySet{comSet1, comSet2, comSet3,
		comSet4, comSet5, comSet6, comSet7, comSet8, comSet9, comSet10} {
		s, _ := NewCommunitySet(c)
		m[c.CommunitySetName] = s
	}

	createCommunityC := func(name string, option gobgp.MatchSetOptionsType) *CommunityCondition {
		matchSet := gobgp.MatchCommunitySet{}
	...
	// ANY case
	p1 := createCommunityC("comset1", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createCommunityC("comset2", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createCommunityC("comset3", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createCommunityC("comset4", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createCommunityC("comset5", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createCommunityC("comset6", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createCommunityC("comset7", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createCommunityC("comset8", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)

	// ALL case
	p9 := createCommunityC("comset9", gobgp.MATCH_SET_OPTIONS_TYPE_ALL)

	// INVERT case
	p10 := createCommunityC("comset10", gobgp.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestCommunityConditionEvaluateWithOtherCondition(t *testing.T) {
	...
	ds.NeighborSets = []gobgp.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []gobgp.AsPathSet{asPathSet}
	ds.BgpDefinedSets.CommunitySets = []gobgp.CommunitySet{comSet1, comSet2}
...
func TestExtCommunityConditionEvaluate(t *testing.T) {
	...
	p1 := createExtCommunityC("ecomSet1", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createExtCommunityC("ecomSet2", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createExtCommunityC("ecomSet3", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createExtCommunityC("ecomSet4", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createExtCommunityC("ecomSet5", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createExtCommunityC("ecomSet6", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createExtCommunityC("ecomSet7", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createExtCommunityC("ecomSet8", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)
	p9 := createExtCommunityC("ecomSet9", gobgp.MATCH_SET_OPTIONS_TYPE_ANY)

	// ALL case
	p10 := createExtCommunityC("ecomSet10", gobgp.MATCH_SET_OPTIONS_TYPE_ALL)

	// INVERT case
	p11 := createExtCommunityC("ecomSet11", gobgp.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestExtCommunityConditionEvaluateWithOtherCondition(t *testing.T) {
	...
	ds.NeighborSets = []gobgp.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []gobgp.AsPathSet{asPathSet}
	ds.BgpDefinedSets.ExtCommunitySets = []gobgp.ExtCommunitySet{ecomSet1, ecomSet2}
...
func TestPolicyMatchAndReplaceMed(t *testing.T) {
	...
	m := "200"
	s := createStatement("statement1", "ps1", "ns1", true)
	s.Actions.BgpActions.SetMed = gobgp.BgpSetMedType(m)
...
func createStatement(name, psname, nsname string, accept bool) gobgp.Statement {
	c := gobgp.Conditions{
	...
	rd := gobgp.ROUTE_DISPOSITION_REJECT_ROUTE
	if accept {
		rd = gobgp.ROUTE_DISPOSITION_ACCEPT_ROUTE
	}
	a := gobgp.Actions{
		RouteDisposition: rd,
	}
...
func createSetCommunity(operation string, community ...string) gobgp.SetCommunity {

	s := gobgp.SetCommunity{
...
func createPolicyDefinition(defName string, stmt ...gobgp.Statement) gobgp.PolicyDefinition {
...
func createRoutingPolicy(ds gobgp.DefinedSets, pd ...gobgp.PolicyDefinition) gobgp.RoutingPolicy {
	pl := gobgp.RoutingPolicy{
		DefinedSets:       ds,
...
func createPrefixSet(name string, prefix string, maskLength string) gobgp.PrefixSet {
	ps := gobgp.PrefixSet{
		PrefixSetName: name,
...
func createNeighborSet(name string, addr string) gobgp.NeighborSet {
...
func TestPrefixSetOperation(t *testing.T) {
	...
	_, err := NewPrefixSet(gobgp.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []gobgp.Prefix{p1, p2},
	})
	assert.NotNil(t, err)
	m1, _ := NewPrefixSet(gobgp.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []gobgp.Prefix{p1},
	})
	m2, err := NewPrefixSet(gobgp.PrefixSet{PrefixSetName: "ps2"})
	assert.Nil(t, err)
	err = m1.Append(m2)
	assert.Nil(t, err)
	err = m2.Append(m1)
	...
	p3, _ := NewPrefix(gobgp.Prefix{IpPrefix: "10.10.0.0/24", MasklengthRange: ""})
	p4, _ := NewPrefix(gobgp.Prefix{IpPrefix: "0::/25", MasklengthRange: ""})
...
func TestPrefixSetMatch(t *testing.T) {
	...
	ps, err := NewPrefixSet(gobgp.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []gobgp.Prefix{p1, p2},
	})
	assert.Nil(t, err)
	...
	ps2, err := NewPrefixSet(gobgp.PrefixSet{
		PrefixSetName: "ps2",
		PrefixList:    []gobgp.Prefix{p3},
	})
	assert.Nil(t, err)
	err = ps.Append(ps2)
	assert.Nil(t, err)
	...
	ps3, err := NewPrefixSet(gobgp.PrefixSet{
		PrefixSetName: "ps3",
		PrefixList:    []gobgp.Prefix{p1},
	})
	assert.Nil(t, err)
...
func TestLargeCommunityMatchAction(t *testing.T) {
	...
	m, err := NewLargeCommunityCondition(gobgp.MatchLargeCommunitySet{
	...
	a, err := NewLargeCommunityAction(gobgp.SetLargeCommunity{
		SetLargeCommunityMethod: gobgp.SetLargeCommunityMethod{
			CommunitiesList: []string{"100:100:100"},
		},
		Options: gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE,
	})
	assert.Equal(t, err, nil)
	p, _ = a.Apply(p, nil)
	...
	a, err = NewLargeCommunityAction(gobgp.SetLargeCommunity{
		...
		Options: gobgp.BGP_SET_COMMUNITY_OPTION_TYPE_ADD,
	})
	assert.Equal(t, err, nil)
	p, _ = a.Apply(p, nil)
...
func TestAfiSafiInMatchPath(t *testing.T) {
	condition, err := NewAfiSafiInCondition([]gobgp.AfiSafiType{gobgp.AFI_SAFI_TYPE_L3VPN_IPV4_UNICAST, gobgp.AFI_SAFI_TYPE_L3VPN_IPV6_UNICAST})
