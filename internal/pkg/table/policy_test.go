...
func TestPrefixCalcurateNoRange(t *testing.T) {
	...
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/24", MasklengthRange: ""})
	match1 := pl1.Match(path)
	assert.Equal(t, true, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/23", MasklengthRange: ""})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
...
func TestPrefixCalcurateAddress(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.11.0.0/16", MasklengthRange: "21..24"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
	match2 := pl2.Match(path)
...
func TestPrefixCalcurateLength(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.64.0/24", MasklengthRange: "21..24"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.64.0/16", MasklengthRange: "21..24"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthRange(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..23"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "25..26"})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/16", MasklengthRange: "21..24"})
	match3 := pl3.Match(path)
...
func TestPrefixCalcurateNoRangeIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: ""})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123:1::/64", MasklengthRange: ""})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
	pl3, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "64..80"})
	match3 := pl3.Match(path)
	assert.Equal(t, true, match3)
}
...
func TestPrefixCalcurateAddressIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:128::/48", MasklengthRange: "64..80"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "64..80"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123:64::/64", MasklengthRange: "64..80"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123:64::/48", MasklengthRange: "64..80"})
	match2 := pl2.Match(path)
	assert.Equal(t, true, match2)
}
...
func TestPrefixCalcurateLengthRangeIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, peer, time.Now())[0]
	// test
	pl1, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "62..63"})
	match1 := pl1.Match(path)
	assert.Equal(t, false, match1)
	pl2, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "65..66"})
	match2 := pl2.Match(path)
	assert.Equal(t, false, match2)
	pl3, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "2001:123:123::/48", MasklengthRange: "63..65"})
	match3 := pl3.Match(path)
	assert.Equal(t, true, match3)
}
...
func TestPolicyNotMatch(t *testing.T) {
	...
	ds.PrefixSets = []bgpconfig.PrefixSet{ps}
	ds.NeighborSets = []bgpconfig.NeighborSet{ns}
...
func TestPolicyDifferentRoutefamilyOfPathAndPolicy(t *testing.T) {
	...
	ds := bgpconfig.DefinedSets{}
	ds.PrefixSets = []bgpconfig.PrefixSet{psIPv4, psIPv6}
	ds.NeighborSets = []bgpconfig.NeighborSet{nsIPv4, nsIPv6}
...
func TestAsPathLengthConditionEvaluate(t *testing.T) {
	...
	asPathLength := bgpconfig.AsPathLength{
		Operator: "eq",
		Value:    5,
	}
...
func TestAsPathConditionEvaluate(t *testing.T) {
	...
	// create match condition
	asPathSet1 := bgpconfig.AsPathSet{
		AsPathSetName: "asset1",
		AsPathList:    []string{"^65001"},
	}

	asPathSet2 := bgpconfig.AsPathSet{
		AsPathSetName: "asset2",
		AsPathList:    []string{"65005$"},
	}

	asPathSet3 := bgpconfig.AsPathSet{
		AsPathSetName: "asset3",
		AsPathList:    []string{"65004", "65005$"},
	}

	asPathSet4 := bgpconfig.AsPathSet{
		AsPathSetName: "asset4",
		AsPathList:    []string{"65000$"},
	}

	asPathSet5 := bgpconfig.AsPathSet{
		AsPathSetName: "asset5",
		AsPathList:    []string{"65010"},
	}

	asPathSet6 := bgpconfig.AsPathSet{
		AsPathSetName: "asset6",
		AsPathList:    []string{"^65010$"},
	}

	m := make(map[string]DefinedSet)
	for _, s := range []bgpconfig.AsPathSet{asPathSet1, asPathSet2, asPathSet3,
		asPathSet4, asPathSet5, asPathSet6} {
		a, _ := NewAsPathSet(s)
		m[s.AsPathSetName] = a
	}

	createAspathC := func(name string, option bgpconfig.MatchSetOptionsType) *AsPathCondition {
		matchSet := bgpconfig.MatchAsPathSet{}
	...
	p1 := createAspathC("asset1", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createAspathC("asset2", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createAspathC("asset3", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createAspathC("asset4", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createAspathC("asset5", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset3", bgpconfig.MATCH_SET_OPTIONS_TYPE_ALL)
	p8 := createAspathC("asset3", bgpconfig.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestMultipleAsPathConditionEvaluate(t *testing.T) {
	...
	p5 := createAspathC("asset5", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset7", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createAspathC("asset8", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p9 := createAspathC("asset9", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
...
func TestAsPathCondition(t *testing.T) {
	...
	for k, v := range tests {
		s, _ := NewAsPathSet(bgpconfig.AsPathSet{
			AsPathSetName: k,
			AsPathList:    []string{k},
		})
		c, _ := NewAsPathCondition(bgpconfig.MatchAsPathSet{
			AsPathSet:       k,
...
func TestAsPathConditionWithOtherCondition(t *testing.T) {
	...
	ds.PrefixSets = []bgpconfig.PrefixSet{ps}
	ds.NeighborSets = []bgpconfig.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []bgpconfig.AsPathSet{asPathSet}
...
func TestAs4PathConditionEvaluateMixedWith2byteAS(t *testing.T) {
	...
	p1 := createAspathC("asset1", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createAspathC("asset2", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createAspathC("asset3", bgpconfig.MATCH_SET_OPTIONS_TYPE_ALL)
	p4 := createAspathC("asset4", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createAspathC("asset5", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createAspathC("asset6", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createAspathC("asset7", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
...
func TestCommunityConditionEvaluate(t *testing.T) {
	...
	comSet10 := bgpconfig.CommunitySet{
	...
	for _, c := range []bgpconfig.CommunitySet{comSet1, comSet2, comSet3,
		comSet4, comSet5, comSet6, comSet7, comSet8, comSet9, comSet10} {
		s, _ := NewCommunitySet(c)
		m[c.CommunitySetName] = s
	}

	createCommunityC := func(name string, option bgpconfig.MatchSetOptionsType) *CommunityCondition {
		matchSet := bgpconfig.MatchCommunitySet{}
	...
	// ANY case
	p1 := createCommunityC("comset1", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createCommunityC("comset2", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createCommunityC("comset3", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createCommunityC("comset4", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createCommunityC("comset5", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createCommunityC("comset6", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createCommunityC("comset7", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createCommunityC("comset8", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)

	// ALL case
	p9 := createCommunityC("comset9", bgpconfig.MATCH_SET_OPTIONS_TYPE_ALL)

	// INVERT case
	p10 := createCommunityC("comset10", bgpconfig.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestCommunityConditionEvaluateWithOtherCondition(t *testing.T) {
	...
	ds.NeighborSets = []bgpconfig.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []bgpconfig.AsPathSet{asPathSet}
	ds.BgpDefinedSets.CommunitySets = []bgpconfig.CommunitySet{comSet1, comSet2}
...
func TestExtCommunityConditionEvaluate(t *testing.T) {
	...
	p1 := createExtCommunityC("ecomSet1", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p2 := createExtCommunityC("ecomSet2", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p3 := createExtCommunityC("ecomSet3", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p4 := createExtCommunityC("ecomSet4", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p5 := createExtCommunityC("ecomSet5", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p6 := createExtCommunityC("ecomSet6", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p7 := createExtCommunityC("ecomSet7", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p8 := createExtCommunityC("ecomSet8", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)
	p9 := createExtCommunityC("ecomSet9", bgpconfig.MATCH_SET_OPTIONS_TYPE_ANY)

	// ALL case
	p10 := createExtCommunityC("ecomSet10", bgpconfig.MATCH_SET_OPTIONS_TYPE_ALL)

	// INVERT case
	p11 := createExtCommunityC("ecomSet11", bgpconfig.MATCH_SET_OPTIONS_TYPE_INVERT)
...
func TestExtCommunityConditionEvaluateWithOtherCondition(t *testing.T) {
	...
	ds.NeighborSets = []bgpconfig.NeighborSet{ns}
	ds.BgpDefinedSets.AsPathSets = []bgpconfig.AsPathSet{asPathSet}
	ds.BgpDefinedSets.ExtCommunitySets = []bgpconfig.ExtCommunitySet{ecomSet1, ecomSet2}
...
func TestPolicyMatchAndReplaceMed(t *testing.T) {
	...
	m := "200"
	s := createStatement("statement1", "ps1", "ns1", true)
	s.Actions.BgpActions.SetMed = bgpconfig.BgpSetMedType(m)
...
func createStatement(name, psname, nsname string, accept bool) bgpconfig.Statement {
	c := bgpconfig.Conditions{
	...
	rd := bgpconfig.ROUTE_DISPOSITION_REJECT_ROUTE
	if accept {
		rd = bgpconfig.ROUTE_DISPOSITION_ACCEPT_ROUTE
	}
	a := bgpconfig.Actions{
		RouteDisposition: rd,
	}
...
func createSetCommunity(operation string, community ...string) bgpconfig.SetCommunity {

	s := bgpconfig.SetCommunity{
...
func createPolicyDefinition(defName string, stmt ...bgpconfig.Statement) bgpconfig.PolicyDefinition {
	pd := bgpconfig.PolicyDefinition{
		Name:       defName,
		Statements: []bgpconfig.Statement(stmt),
	}
	return pd
}

func createRoutingPolicy(ds bgpconfig.DefinedSets, pd ...bgpconfig.PolicyDefinition) bgpconfig.RoutingPolicy {
	pl := bgpconfig.RoutingPolicy{
		DefinedSets:       ds,
...
func createPrefixSet(name string, prefix string, maskLength string) bgpconfig.PrefixSet {
...
func createNeighborSet(name string, addr string) bgpconfig.NeighborSet {
...
func TestPrefixSetOperation(t *testing.T) {
	...
	_, err := NewPrefixSet(bgpconfig.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []bgpconfig.Prefix{p1, p2},
	})
	assert.NotNil(t, err)
	m1, _ := NewPrefixSet(bgpconfig.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []bgpconfig.Prefix{p1},
	})
	m2, err := NewPrefixSet(bgpconfig.PrefixSet{PrefixSetName: "ps2"})
	assert.Nil(t, err)
	err = m1.Append(m2)
	assert.Nil(t, err)
	err = m2.Append(m1)
	...
	p3, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "10.10.0.0/24", MasklengthRange: ""})
	p4, _ := NewPrefix(bgpconfig.Prefix{IpPrefix: "0::/25", MasklengthRange: ""})
...
func TestPrefixSetMatch(t *testing.T) {
	...
	ps, err := NewPrefixSet(bgpconfig.PrefixSet{
		PrefixSetName: "ps1",
		PrefixList:    []bgpconfig.Prefix{p1, p2},
	})
	assert.Nil(t, err)
	...
	ps2, err := NewPrefixSet(bgpconfig.PrefixSet{
		PrefixSetName: "ps2",
		PrefixList:    []bgpconfig.Prefix{p3},
	})
	assert.Nil(t, err)
	err = ps.Append(ps2)
	assert.Nil(t, err)
	...
	ps3, err := NewPrefixSet(bgpconfig.PrefixSet{
		PrefixSetName: "ps3",
		PrefixList:    []bgpconfig.Prefix{p1},
	})
	assert.Nil(t, err)
...
func TestLargeCommunityMatchAction(t *testing.T) {
	...
	m, err := NewLargeCommunityCondition(bgpconfig.MatchLargeCommunitySet{
	...
	a, err := NewLargeCommunityAction(bgpconfig.SetLargeCommunity{
		SetLargeCommunityMethod: bgpconfig.SetLargeCommunityMethod{
			CommunitiesList: []string{"100:100:100"},
		},
		Options: bgpconfig.BGP_SET_COMMUNITY_OPTION_TYPE_REMOVE,
	})
	assert.Equal(t, err, nil)
	p, _ = a.Apply(p, nil)
	...
	a, err = NewLargeCommunityAction(bgpconfig.SetLargeCommunity{
		...
		Options: bgpconfig.BGP_SET_COMMUNITY_OPTION_TYPE_ADD,
	})
	assert.Equal(t, err, nil)
	p, _ = a.Apply(p, nil)
...
func TestAfiSafiInMatchPath(t *testing.T) {
	condition, err := NewAfiSafiInCondition([]bgpconfig.AfiSafiType{bgpconfig.AFI_SAFI_TYPE_L3VPN_IPV4_UNICAST, bgpconfig.AFI_SAFI_TYPE_L3VPN_IPV6_UNICAST})
...
func TestMultipleStatementPolicy(t *testing.T) {
	r := NewRoutingPolicy(logger)
	rp := bgpconfig.RoutingPolicy{
		PolicyDefinitions: []bgpconfig.PolicyDefinition{{
			Name: "p1",
