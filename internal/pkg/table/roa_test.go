...
func validateOne(rt *ROATable, cidr, aspathStr string) bgpconfig.RpkiValidationResultType {
...
func TestValidate0(t *testing.T) {
	...
	assert.Equal(r, bgpconfig.RPKI_VALIDATION_RESULT_TYPE_VALID)

	r = validateOne(table, "192.168.0.0/24", "100 200")
	assert.Equal(r, bgpconfig.RPKI_VALIDATION_RESULT_TYPE_VALID)

	r = validateOne(table, "192.168.0.0/24", "300")
	assert.Equal(r, bgpconfig.RPKI_VALIDATION_RESULT_TYPE_INVALID)
...
func TestValidate2(t *testing.T) {
	...
	var r bgpconfig.RpkiValidationResultType

	r = validateOne(table, "10.0.0.0/16", "65000")
	assert.Equal(r, bgpconfig.RPKI_VALIDATION_RESULT_TYPE_NOT_FOUND)
