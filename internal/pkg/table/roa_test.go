...
func validateOne(rt *ROATable, cidr, aspathStr string) gobgp.RpkiValidationResultType {
...
func TestValidate0(t *testing.T) {
	...
	assert.Equal(r, gobgp.RPKI_VALIDATION_RESULT_TYPE_VALID)

	r = validateOne(table, "192.168.0.0/24", "100 200")
	assert.Equal(r, gobgp.RPKI_VALIDATION_RESULT_TYPE_VALID)

	r = validateOne(table, "192.168.0.0/24", "300")
	assert.Equal(r, gobgp.RPKI_VALIDATION_RESULT_TYPE_INVALID)
...
func TestValidate2(t *testing.T) {
	...
	var r gobgp.RpkiValidationResultType

	r = validateOne(table, "10.0.0.0/16", "65000")
	assert.Equal(r, gobgp.RPKI_VALIDATION_RESULT_TYPE_NOT_FOUND)
